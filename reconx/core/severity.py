...
class Finding:
    ...
    title: str
    detail: str = ""
    module: str = ""   # "dns" | "ssl" | "http" | "ports" | "udp" | "whois" | "ip_intel"
...
def make_finding(title: str, detail: str = "", module: str = "") -> Finding:
    """Create a Finding with auto-classified severity."""
    return Finding(
...
def aggregate_findings(collected: dict) -> list[Finding]:
    ...
    to typed Finding objects with severity scores.
    """
    ...
    if dns:
        _add(getattr(dns, "security_findings", dns.get("security_findings", []) if isinstance(dns, dict) else []), "dns")
    ...
    # SSL
    ssl = collected.get("ssl")
    if ssl:
        ssl_findings = getattr(ssl, "findings", ssl.get("findings", []) if isinstance(ssl, dict) else [])
    ...
    if scan:
        for p in (getattr(scan, "open_ports", scan.get("open_ports", []) if isinstance(scan, dict) else [])):
            ...
            if svc in ("Telnet", "FTP"):
                findings.append(make_finding(f"{svc} open on TCP/{port} — unencrypted protocol", module="ports"))
            elif svc in ("Docker", "MongoDB", "Redis", "Elasticsearch"):
                findings.append(make_finding(f"{svc} open on TCP/{port} — may lack authentication", module="ports"))

    # IP intel
    ip_intel = collected.get("ip_intel")
    if ip_intel:
        _add(getattr(ip_intel, "findings", ip_intel.get("findings", []) if isinstance(ip_intel, dict) else []), "ip_intel")
